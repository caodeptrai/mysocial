...
const Left = () => {
    ...
    useEffect(() => {
        const unsub = setPropose(
            following.length > 0 ? followers.filter((x) => !following.find((y) => y.uid === x.uid)) : followers,
        );
        return unsub;
    }, [followers, following]);
                ...
                {propose?.map((item, index) => {
                    return (
                        <li className="propose-item" key={item.uid} onClick={() => handleShowUserInfo(item)}>
                            ...
                            <button className="propose-btn-fl" onClick={() => handleFollow(item)}>
                                Theo dõi
                            </button>
                        </li>
                    );
